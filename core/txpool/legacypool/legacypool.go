...
func (pool *LegacyPool) ToJournal() map[common.Address]types.Transactions {
	...
	for addr, queued := range pool.queue.queued {
...
func (pool *LegacyPool) promoteExecutables(accounts []common.Address) []*types.Transaction {
	...
	// QKC: use a balance getter that includes soul gas token balance
	getBalance := func(addr common.Address) *uint256.Int {
		...
		return balance.Add(balance, sgtBalance)
	}
	promotable, dropped, removedAddresses := pool.queue.promoteExecutables(accounts, gasLimit, pool.currentState, pool.pendingNonces, getBalance)
