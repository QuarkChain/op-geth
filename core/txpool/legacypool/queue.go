...
// balanceGetter returns the balance for an address that should be used for filtering
// transactions based on affordability.
type balanceGetter func(addr common.Address) *uint256.Int
...
func (q *queue) promoteExecutables(accounts []common.Address, gasLimit uint64, currentState *state.StateDB, nonces *noncer, getBalance balanceGetter) ([]*types.Transaction, []common.Hash, []common.Address) {
	...
	for _, addr := range accounts {
		...
		drops, _ := list.Filter(getBalance(addr), gasLimit)
